...
class Player(Entity):
	def __init__(self, game, zone, pos, groups):
		...
		self.on_stairs_left_up, self.on_stairs_right_up, self.on_stairs_right_down, self.on_stairs_left_down\
	...
	def dash(self):
		...
		if self.game.actions['right_click'] and not self.dashing:
			if self.can_dash:
				self.dash_start_pos = self.rect.center
	...
	def input(self):
		...
		if not self.attacking and not self.dashing:
			...
			if keys[pygame.K_LEFT] or keys[pygame.K_a]:
				if pygame.sprite.spritecollide(self, self.zone.stairs_right_up_sprites, False) or pygame.sprite.spritecollide(self, self.zone.stairs_left_down_sprites, False):
					self.on_stairs_right_up = True
					if keys[pygame.K_UP]:
						self.vec.y = 0
				...
				elif pygame.sprite.spritecollide(self, self.zone.stairs_right_down_sprites, False) or pygame.sprite.spritecollide(self, self.zone.stairs_left_up_sprites, False):
					self.on_stairs_right_down = True
					if keys[pygame.K_UP]:
						self.vec.x = -1
					elif keys[pygame.K_DOWN]:
						self.vec.y = -1
	...
	def get_distance_direction_and_angle(self):
		...
		mouse_vec = pygame.math.Vector2((self.mx + self.zone.visible_sprites.offset[0], self.my + self.zone.visible_sprites.offset[1]))
	...
	# icluded in player input method, but enemies do not rely on key input so separate one for enemies based on the below method...
	def stair_collisions(self, direction):
		keys = pygame.key.get_pressed() 
		...
		# 	if sprite.rect.colliderect(self.hitbox):
				...
				# rel_x = self.hitbox.x - sprite.rect.x

				# if sprite in self.zone.ramp_right_up_sprites:
				# 	pos_height = rel_x + self.hitbox.width
				# elif sprite in self.zone.ramp_left_up_sprites:
				# 	pos_height = rel_x + self.hitbox.width

				# pos_height = min(pos_height, self.game.TILESIZE)
				# pos_height = max(pos_height, 0)

				# target_y = sprite.rect.y + self.game.TILESIZE - pos_height

				# if self.hotbox.bottom > target_y:
